...
const HeroImageField = ({ inputDetails, endpoint, name }) => {
  ...
  useEffect(() => {
    if (inputDetails?.hero) {
      ...
      setInputKeys(inputDetails.hero.map(() => Date.now())); // Reset keys
    }
  }, [inputDetails]);
  ...
  const clearImage = (index) => {
    setImagePreviews((prev) => {
      const updated = [...prev];
      updated[index] = inputDetails?.hero?.[index] || null; // Reset to original image or null
      return updated;
    });
  ...
  if (imagePreviews.length === 0) {
    return (
      <>
        <p className="w-100 text-md text-white font-bold mb-3">Hero Images</p>
